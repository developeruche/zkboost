...
pub fn verify_minisig(signature_str: &str, public_key_str: &str, program: &[u8]) -> Result<(), anyhow::Error> {
    
    let signature = SignatureBox::from_string(signature_str)?;
    
    let public_key_str_clean = if public_key_str.contains("\n") {
        public_key_str.lines().last().unwrap_or(public_key_str).trim()
    } else {
        public_key_str.trim()
    };
...
mod tests {
    ...
    fn test_verify_minisig_generated_keys() {
        ...
        assert!(result.is_ok(), "Verification failed for generated keys: {:?}", result.err());
    }
    ...
    async fn test_verify_minisig_downloaded_airbender() {
        ...
        assert!(result.is_ok(), "Verification failed for Airbender: {:?}", result.err());
    }
    ...
    async fn test_verify_minisig_airbender_hardcoded_key() {
        ...
        let result = verify_minisig(sig_str.trim(), pub_key_str.trim(), &program_bytes);
        assert!(result.is_ok(), "Verification failed for Airbender (hardcoded key): {:?}", result.err());
    }
}