...
impl ProgramInstance {
    ...
    pub fn new(
        ...
        zkvm_vendor_name: String,
    ) -> Result<Self, anyhow::Error> {
        let zkvm_vendor = zkVMVendor::from_str(&zkvm_vendor_name)
            .map_err(|e| anyhow::anyhow!("Failed to parse zkVM vendor name: {}", e))?;
...
pub fn load_elf_from_disk(program_path: impl AsRef<Path>) -> Result<Vec<u8>, anyhow::Error> {
    ...
    let elf_bytes = fs::read(path)
        .map_err(|e| format!("Failed to read SP1 ELF from '{}': {}", path.display(), e))
